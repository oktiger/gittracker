...
export function RunTargetsDialog({
  projectId,
  projectName,
  mode,
  initialTargets = [],
    ...
    mode === "config" || mode === "confirm"
      ? toDraft(
          initialTargets.length
            ? initialTargets
            : [{ id: "", name: "开发", cwd: ".", command: "npm run dev", isDefault: true }],
        )
      : [],
  );
  ...
  const setDefault = (idx: number) => {
    setDrafts((prev) =>
      prev.map((d, i) => ({ ...d, isDefault: i === idx, checked: i === idx ? true : d.checked })),
    );
  };
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose} role="presentation">
      <div
        className={`dialog dialog-wide${phase === "loading" ? " dialog-ai-live" : ""}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="dialog-header">
          <h3 id="run-targets-title">{title}</h3>
          <button type="button" className="btn-ghost btn-icon" onClick={onClose} disabled={saving}>
            ×
          </button>
        </header>
        ...
        {phase === "loading" ? (
          <>
            <p className="dialog-hint">
              AI 正在只读分析项目结构与脚本；下方会实时显示过程（不会自动执行命令）。
            </p>
          ...
          <>
            <p className="dialog-hint">
              {mode === "config"
                ? `编辑「${projectName}」的启动目标。保存后可从运行菜单选择。`
                : `已为「${projectName}」准备建议。请勾选需要保留的项，可改目录与命令。`}
            </p>
            {error && (
              <p className={error.includes("已改用本地") ? "dialog-warn" : "dialog-error"}>
                {error}
              </p>
            )}
                  ...
                  <div className="run-target-fields">
                    ...
                    <label className="run-field">
                      ...
                      <input
                        ...
                        placeholder="一句话说明用途，例如：打开已打包好的 macOS 应用"
                      />
                    </label>
                    <label className="run-field">
                      <span>目录</span>
