    ...
    let prompt = if locale.is_zh() {
        format!(
            "你是 Git 集成代理。当前目录是隔离的临时集成 Worktree，基于 {default_branch} 创建；只允许在这里工作。\n\
             ...
             严格规则：\n\
             1. 用户已明确选择本批全部 PR（其中可能包含 Draft）。先检查每个 PR 的 diff、依赖及文件重叠，按依赖关系决定最安全的合并顺序，不能仅按 PR 编号排序；不能静默跳过任一 PR。\n\
             ...
             7. 本批处理是原子的：只有全部 PR 都能安全合入才允许继续；任一 PR 无法安全完成时，立即停止，保留当前集成 Worktree，main 不得改变，并在最终回复中说明原因；不要伪造成功。\n\
        ...
        format!(
            "You are a Git integration agent. The current directory is an isolated temporary integration worktree based on {default_branch}; work only here.\n\
             ...
             Rules:\n\
             1. The user explicitly selected every PR in this batch, which may include Drafts. Inspect each PR's diff, dependencies, and file overlap, then choose the safest merge order based on dependencies, not PR number; never silently skip a PR.\n\
             ...
             7. This batch is atomic: continue only if every PR can be safely integrated. If any PR cannot be completed safely, stop, preserve this integration worktree, leave main unchanged, and explain why. Never claim success.\n\
